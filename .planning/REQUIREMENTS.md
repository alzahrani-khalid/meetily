...
- [x] **UI-03**: User switches language and the app triggers a full reload; the selection persists across restarts (§12.4 — reload, not hot-swap)
- [x] **UI-04**: User sees Arabic text rendered right-to-left in Tajawal (loaded via `next/font/google` alongside existing `Source_Sans_3`)
...
- [x] **SUMM-04**: User can view an Arabic summary in the BlockNote editor — editable if the §7 spike confirms RTL support, read-only rendered markdown fallback if the spike finds blockers (§12.3 decision)
...
- [x] **TPL-01**: Meeting template loader resolves `{id}.{locale}.json` first, falls back to `{id}.json`, at every tier of the existing custom → bundled → builtin chain in `frontend/src-tauri/src/summary/templates/loader.rs`
- [x] **TPL-02**: All 6 meeting templates are embedded in `defaults.rs`, fixing the pre-existing 2/6 embed gap (currently only `DAILY_STANDUP` and `STANDARD_MEETING` are embedded)
