...
## Karpathy Skills — Core Coding Principles
...
Advisory input only — the verdict is a keyword heuristic; the final call is yours with full phase context, and you draft `fanout-tasks.json` from the *parallel-safe subset* only (dependent tasks stay sequential in GSD). Never auto-fanout, and never fan out `/gsd-verify-work` or `/gsd-secure-phase` (single-context reasoning, not parallel breadth). Full flow: `~/.claude/skills/gsd-fanout/SKILL.md`.
<!-- fanout:gsd-rule-end -->
